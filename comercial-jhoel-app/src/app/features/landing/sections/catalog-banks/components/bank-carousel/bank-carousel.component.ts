...
/**
 * Carousel 3D tipo "coverflow" para Bancos — misma mecánica que
 * `PhoneCarouselComponent`/`NewsCarouselComponent`/
 * `CatalogProductCarouselComponent` (una card activa centrada, vecinas
 * parciales en profundidad, autoplay, swipe táctil/mouse vía Pointer
 * Events, teclado), copiada localmente siguiendo la misma convención ya
 * establecida en este proyecto.
 */
@Component({
  selector: 'app-bank-carousel',
  standalone: true,
  imports: [IconComponent, BankCardComponent],
  templateUrl: './bank-carousel.component.html',
  styleUrl: './bank-carousel.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BankCarouselComponent implements OnInit {
  ...
  get isLast(): boolean {
    return this.activeIndex() >= this.banks.length - 1;
  }
  ...
  private advanceAutoplay(): void {
    ...
    this.activeIndex.update((i) => (i + 1) % this.banks.length);
  }

  private startAutoplay(): void {
    if (this.autoplayTimer !== null || this.banks.length <= 1) {
      return;
    }
