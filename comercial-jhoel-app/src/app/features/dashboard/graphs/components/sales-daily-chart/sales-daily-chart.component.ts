...
import { SalesDailyStat } from '../../../../../core/models';
...
/**
 * "Ventas del mes" en Gráficas → Indicadores de Ventas — reutiliza
 * `SalesService.getDailyStats()`. Monto total vendido (Q), no cantidad de
 * ventas — confirmado con el usuario.
 */
@Component({
  selector: 'app-sales-daily-chart',
  standalone: true,
  imports: [LineChartComponent],
  templateUrl: './sales-daily-chart.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SalesDailyChartComponent {
  private readonly salesService = inject(SalesService);

  readonly loading = signal(true);
  readonly loadError = signal(false);
  private readonly days = signal<SalesDailyStat[]>([]);
