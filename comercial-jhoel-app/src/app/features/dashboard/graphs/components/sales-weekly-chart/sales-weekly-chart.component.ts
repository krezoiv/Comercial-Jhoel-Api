...
import { SalesWeeklyStat } from '../../../../../core/models';
...
/**
 * "Ventas por semana" en Gráficas → Indicadores de Ventas — reutiliza
 * `SalesService.getWeeklyStats()`. Buckets fijos de 7 días desde el día 1
 * del mes actual, nunca semana ISO — se reinicia solo cada mes.
 */
@Component({
...
export class SalesWeeklyChartComponent {
  ...
  private readonly weeks = signal<SalesWeeklyStat[]>([]);
  ...
  fetch(): void {
    ...
    this.salesService.getWeeklyStats().subscribe({
      next: (stats) => {
