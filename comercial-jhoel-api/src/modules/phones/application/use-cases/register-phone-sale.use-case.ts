...
/**
 * Venta — registers the sale of one `DISPONIBLE` phone. The client is
 * validated here via `ClientsModule`'s own exported `CLIENT_REPOSITORY`
 * (the clean route that module's own doc comment invites), rather than
 * SIM's raw-SQL-across-module-boundary check — `register_phone_sale` still
 * re-validates it server-side too (defense in depth, never trusts this
 * TypeScript check alone). The phone-availability check and the actual
 * state transition both happen atomically inside `register_phone_sale`
 * (see the migration's own doc comment) — nothing here duplicates that.
 */
@Injectable()
