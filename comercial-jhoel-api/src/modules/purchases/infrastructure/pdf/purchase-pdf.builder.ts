...
export interface PurchasePdfSupplier {
  name: string;
  phone: string | null;
  address: string | null;
  taxId: string | null;
...
const COLUMNS = [
  ...
  { header: 'Producto', width: 200, align: 'left' as const },
  { header: 'Presentación', width: 90, align: 'left' as const },
  { header: 'Costo unitario', width: 90, align: 'right' as const },
  { header: 'Total', width: 75, align: 'right' as const },
];

/**
 * Reconstructs a Compra invoice purely from already-persisted data
 * (`PurchaseOutput` + `Supplier` + company settings) — never re-runs
 * `confirm_purchase` or touches inventory. Normal A4 document layout,
 * structurally mirrors `sale-pdf.builder.ts`.
 */
export function buildPurchasePdf(options: PurchasePdfOptions): Promise<Buffer> {
...
function drawDocumentInfo(
  ...
  doc.text(
    `Tipo de pago: ${options.paymentType === 'CREDITO' ? 'Crédito' : 'Contado'}`,
    startX,
    doc.y,
    { width: usableWidth },
  );
  if (options.paymentType === 'CREDITO' && options.paymentDueDate) {
    doc.text(
      `Fecha de vencimiento: ${options.paymentDueDate}`,
      startX,
      doc.y,
      { width: usableWidth },
    );
  ...
  doc
    .fontSize(10)
    .fillColor('#0f172a')
    .text('Proveedor', startX, doc.y, { width: usableWidth });
  doc.fontSize(9).fillColor('#334155');
  doc.text(options.supplier.name, startX, doc.y, { width: usableWidth });
  const supplierLines = [
    options.supplier.phone ? `Tel: ${options.supplier.phone}` : null,
    options.supplier.address,
    options.supplier.taxId ? `NIT: ${options.supplier.taxId}` : null,
  ].filter((line): line is string => !!line);
  for (const line of supplierLines) {
    doc.text(line, startX, doc.y, { width: usableWidth });
  }
...
function drawTable(doc: PDFKit.PDFDocument, items: PurchasePdfItem[]): void {
  const startX = doc.page.margins.left;
