...
export class TypeOrmPhoneRepository implements PhoneRepository {
  ...
  async findBySimNumber(simNumber: string): Promise<Phone | null> {
    const orm = await this.repository.findOne({ where: { simNumber } });
    return orm ? PhoneMapper.toDomain(orm) : null;
  }
  ...
  private translateUniqueViolation(
    ...
    if (error instanceof QueryFailedError) {
      ...
      if (constraint === 'UQ_phones_sim_number') {
        return new SimNumberAlreadyExistsError(data.simNumber);
      }
    }
    return error;
  }
